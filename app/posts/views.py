...
def api_post(id):
    ...
    post_data = {
        'id': post.id,
        'title': post.title,
        'type': post.type.name,  # Assuming you have a name attribute or use .value
        'image_url': url_for('static', filename='posts_images/' + post.image_file),
...
def view_posts():
    ...
    posts_pagination = Posts.query.order_by('id', 'created').paginate(page=page, per_page=5)
    posts = posts_pagination.items

    if posts:
        previous_post_id = posts[-1].id
        next_post_id = posts[1].id if len(posts) > 1 else posts[0].id
    else:
        previous_post_id = next_post_id = None
